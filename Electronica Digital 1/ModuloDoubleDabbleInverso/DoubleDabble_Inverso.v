...
module DoubleDabble_Inverso (
    ...
    .in_bcd(bcd_subs),.arr_corr(bcd_reg),.A(A));

    Comp u_Comp_a1 (.a(bcd_reg[3:0]),.mayor(m_a1));
    Comp u_Comp_a2 (.a(bcd_reg[7:4]),.mayor(m_a2));
    Comp u_Comp_a3 (.a(bcd_reg[11:8]),.mayor(m_a3));

    Substrac u_Sub_a1 (.a(bcd_reg[3:0]),.S(bcd_subs[3:0]));
    Substrac u_Sub_a2 (.a(bcd_reg[7:4]),.S(bcd_subs[7:4]));
    Substrac u_Sub_a3 (.a(bcd_reg[11:8]),.S(bcd_subs[11:8]));

    Increase u_Increase (.clk(clk),.load(load),.increase(increase),.i_eq_n(i_eq_n));

    Control_DoubleDabble_Inverso u_Control (.clk(clk),.rst(rst),.init(init),.i_eq_n(i_eq_n),.load(load),
    .shift(shift),.subs(subs),.increase(increase),.done(done));
